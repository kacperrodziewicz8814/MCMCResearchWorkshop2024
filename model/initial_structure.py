...
class SARWalk:
    ...
    def generate_walk(self):
        ...
        potential_moves = [np.array(move) for move in [(self.step_size, 0, 0), (-self.step_size, 0, 0), (0, self.step_size, 0), (0, -self.step_size, 0), (0, 0, self.step_size), (0, 0, -self.step_size)]]
        visited_positions = set(walk)
        steps_taken = 1
        ...
        while steps_taken < self.number_of_steps:
            ...
            for move in potential_moves:
                ...
                if tuple(new_position) not in visited_positions:  # Check if position was visited
        ...
        if not move_made:
            ...
            walk.pop()
            position = np.array(walk[-1])
            visited_positions.remove(tuple(position))
            steps_taken -= 1
...
class CondesedStructure:
    def __init__(self, number_of_steps):
        self.number_of_steps = number_of_steps

    def generate_structure(self):
        position = np.array([0,0,0]) # Starting position
        structure = [tuple(position)]

    def get_available_points(self, structure):
        pass
